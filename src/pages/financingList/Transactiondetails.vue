    ...
    <div class="wrap">
        ...
        <div class="t-tab">
            ...
            <div class="divTab" v-show="nowIndex===1">
                ...
                <div class="t-content main-body" :style="{'-webkit-overflow-scrolling': scrollMode}">
                    ...
                    <v-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore1">
                        <div>
                            <ul>
                                <li v-for="(item,index) in pageList1" :key="index">
                                    <h5>{{item.TRANS_TYPE_NAME}}</h5>
                                    <p>
                                        <span>{{item.TRANS_TIEM | timerFormat}}</span>
                                        <em>¥{{item.TRANS_AMT | formatNum}}</em>
            ...
            <div class="divTab" v-show="nowIndex===2">
                <div class="t-content main-body" :style="{'-webkit-overflow-scrolling': scrollMode}">
                    <!-- <h4>本月</h4> -->
                    <v-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore2">
                        <div>
                            <ul>
                                <li v-for="(item,index) in pageList2" :key="index">
                                    <h5>{{item.TRANS_TYPE_NAME}}</h5>
                                    <p>
                                        <span>{{item.TRANS_TIEM | timerFormat}}</span>
                                        <em>¥{{item.TRANS_AMT | formatNum}}</em>
            ...
            <div class="divTab" v-show="nowIndex===3">
                ...
                <div class="t-content main-body" :style="{'-webkit-overflow-scrolling': scrollMode}">
                    <!-- <h4>本月</h4> -->
                    <v-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore3">
                        <div>
                            <ul>
                                <li v-for="(item,index) in pageList3" :key="index">
                                    <h5>{{item.TRANS_TYPE_NAME}}</h5>
                                    <p>
                                        <span>{{item.TRANS_TIEM | timerFormat}}</span>
                                        <em>¥{{item.TRANS_AMT | formatNum}}</em>
...
export default {
  ...
  methods: {
    ...
    loadPageList: function() {
      ...
      this.searchCondition2.pageNo = "1";
      this.searchCondition3.pageNo = "1";
      ...
      if (this.nowIndex == 0) {
        ...
        this.endDate = this.getLastMonthYestdy(0);
        this.startDate = this.getLastMonthYestdy(2);
        this.apiQryTradeHis(this.startDate, this.endDate); //交易数据
      } else if (this.nowIndex == 2) {
        this.endDate = this.getLastMonthYestdy(0);
        this.startDate = this.getLastMonthYestdy(3);
        this.apiQryTradeHis(this.startDate, this.endDate); //交易数据
      } else {
        this.endDate = this.getLastMonthYestdy(0);
        this.startDate = this.getLastMonthYestdy(1);
    ...
    more: function() {
      if (this.nowIndex == 0) {
        ...
        API.financial.apiQryTradeHis(data, res => {
          ...
          if (res.PAGE.retList.length < this.searchCondition1.pageSize) {
            this.allLoaded = true;
            Bus.$emit(BusName.showToast, "数据全部加载完成");
          }
        });
        ...
        this.startDate = this.getLastMonthYestdy(3);
        // 分页查询
        this.searchCondition2.pageNo =
          "" + (parseInt(this.searchCondition2.pageNo) + 1);
        let data = {
          currentPage: this.searchCondition2.pageNo,
          // START_DATE: "2018-01-01",
          // END_DATE:"2018-02-25"
          START_DATE: this.startDate,
          END_DATE: this.endDate
        ...
        API.financial.apiQryTradeHis(data, res => {
          this.pageList2 = this.pageList2.concat(res.PAGE.retList);
          if (res.PAGE.retList.length < this.searchCondition2.pageSize) {
            this.allLoaded = true;
            Bus.$emit(BusName.showToast, "数据全部加载完成");
          }
        });
        ...
        // 分页查询
        this.searchCondition3.pageNo =
          "" + (parseInt(this.searchCondition3.pageNo) + 1);
        let data = {
          currentPage: this.searchCondition3.pageNo,
          // START_DATE: "2018-01-01",
          // END_DATE:"2018-02-25"
          START_DATE: this.startDate,
          END_DATE: this.endDate
        ...
        API.financial.apiQryTradeHis(data, res => {
          this.pageList3 = this.pageList3.concat(res.PAGE.retList);
          if (res.PAGE.retList.length < this.searchCondition3.pageSize) {
            this.allLoaded = true;
            Bus.$emit(BusName.showToast, "数据全部加载完成");
          }
        });
    ...
    apiQryTradeHis(start, end) {
      ...
      API.financial.apiQryTradeHis(data, res => {
        if (this.nowIndex == 0) {
          ...
          this.pageList2 = res.PAGE.retList;
        ...
        // alert(JSON.stringify(res.PAGE.retList));
        //alert(res);
      });
    },
    toggleTabs(index) {
