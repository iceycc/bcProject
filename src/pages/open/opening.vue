<template>
    <div class="warp">
        ...
        <div class="opening_box">
            ...
            <section>
                <span> 身份证号</span>
                <input class="inputBox2" type="text" name="text1" placeholder="请输入15-18位身份证号" v-model="data.USER_CARD_ID" @bulr="checkID">
            </section>
            <section>
                <span>职业</span>
                <js-select
        ...
        <p :class="{'bang':true,'no':agree == false}"
           @click="doAgree">
            <span>我已阅读并同意注册</span>
            <a href="javscript:;" @click.stop="showPage('2')" style=" color:#0096FE;">《用户授权服务协议》</a>
            <a href="javscript:;" @click.stop="showPage('1')" style=" color:#0096FE;">《晋商银行直销银行电子账户服务协议》</a>
        </p>
        <section v-show="page" class="page">
            <div class="docs" v-if="showType == 1"><iframe :src="agreeMentSrc" class="indocs"></iframe></div>
            <div class="docs" v-if="showType == 2" style="padding: 20px 20px 0">
                <h2  style="text-align:center">用户授权服务协议</h2>
                <h5>一、概述</h5>
                <p>《用户授权服务协议》（以下简称“本协议＂）是北京比财数据科技有限公司（以下简称“本公司＂）与用户（以下简称“您”）所订立的有效合约。您通过网络页面点击确认或以其他方式选择接受本协议，即表示您与本公司已达成协议并同意接受本协议的全部约定内容。在接受本协议之前，请您仔细阅读本协议的全部内容（特别是以粗体下划线标注的内容）。如您不同意本协议的内容，或无法准确理解本协议任何条款的含义，请不要进行确认及后续操作。如果您对本协议的条款有疑问的，请拨打客服电话（86-10
                ...
                <h5>三、授权条款</h5>
                <p>3．1您授权本公司及其关联公司向您在本服务页面选择的第三方传递您的信息，该等信息的提供以第三方为提供服务所需。页面提示上可能会展示具体授权对象、授权字段名称、范围等，授权字段通过加密通道传递给第三方。本公司会要求该第三方依法使用您的上述信息，
                    并应对您的信息保密。</p>
                <p>3．2您理解，本公司是中立平台的提供者，第三方服务由该第三方独立运营并独立承担全部责任。因该第三方服务或其使用您的信息产生的纠纷，或第三方服务违反相关法律法规或本协议约定，或您在使用第三方服务过程中遭受的损失，请您和第三方协商解決。</p>
                <p>3．3您授权本公司在您使用本服务期间或本服务终止后，基于为更好的向您提供服务之目的或向您推广其他产品服务的需要，有权保留并使用您在使用本服务期间所形成的相关信息（包括您选择的第三方合法收集、保存的您的相关信息）并提供给本公司的关联公司。</p>
                <p>3．4您同意本协议中的授权不可变更、撤回或撤销。</p>

                <h5>四、其他条款</h5>
                <p>4．1您同意，本公司有权随时对本协议内容进行单方面的变更，并通过本公司公开渠道发布通知/公告的方式进行公告，所有上述通知/公告等为本协议不可分割的组成部分。该等变更将在通知/公告载明的生效时间生效；若您在本协议内容变更生效后继续使用本公司服务的，表示您已充分阅读、理解并接受变更修改后的协议内容，也将遵循变更修改后的协议内容；若您不同意变更修改后的协议内容，您应立即停止使用本协议项下的服务。</p>
